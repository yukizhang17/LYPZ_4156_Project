...
class Test_Testsubscribe(unittest.TestCase):
	...
	def test_get_subscribe_input(self):
		...
		form1 = {"product": "ps5", "type": "keyword", "expected_price": "10.99"}
		respond1 = (200, {"product": "ps5", "type": "keyword", "platform": None, "expected_price":10.99})
		...
		respond2 = (200, {"product": "ps5", "type": "keyword", "platform": None, "expected_price": None})
		...
		# Test correct input 
		form4 = {"product": "B07H39W49R", "type": "productID", "platform": "Amazon"}
		respond4 = (200, {"product": "B07H39W49R", "type": "productID", "platform": "Amazon", "expected_price": None})
	...
	def test_subscribe_product_keyword(self):
		...
		self.assertEqual(subscribe_product(uid, product, type, platform, expected_price), respond1) 
		
		# Test update price
		expected_price = 12.99
		respond2 = (200, "Subscripted expected price updated!")
		self.assertEqual(subscribe_product(uid, product, type, platform, expected_price), respond2)  

		# Test repeated subscribe 
		respond3 = (400, "User has subscribed the same keyword!")
		self.assertEqual(subscribe_product(uid, product, type, platform, expected_price), respond3) 

		# Test another user subscribe the same keyword
		uid2 = "3333"
		respond4 = (200, "Subscribed successfully!")
		self.assertEqual(subscribe_product(uid2, product, type, platform, expected_price), respond4)  

		# Test missing keyword
		product = None
		respond5 = (400, "missing keyword")
		self.assertEqual(subscribe_product(uid, product, type, platform, expected_price), respond5)  
	...
	def test_subscribe_product_productID(self):
		...
		self.assertEqual(subscribe_product(uid, product, type, "Amazon1", expected_price), respond2)  
		...
		# self.assertEqual(subscribe_product(uid, product, type, platform, expected_price), respond4)  

		# # Test another user subscribe the same product
		# uid2 = "3333"
		# respond5 = (200, "Subscribed successfully!")
		# self.assertEqual(subscribe_product(uid2, product, type, platform, expected_price), respond5)  
	...
	def test_get_unsubscribe_input(self):
		...
		respond1 = (200, {"product": "ps5", "type": "keyword", "platform": None})
		...
		# Test correct input 
		form4 = {"product": "B07H39W49R", "type": "productID", "platform": "Amazon"}
		respond4 = (200, {"product": "B07H39W49R", "type": "productID", "platform": "Amazon"})
	...
	def test_unsubscribe_product_keyword(self):
		...
		self.assertEqual(unsubscribe_product(uid, "ps7", type, platform), respond1) 
		
		# Test user never subscribe this keyword
		respond2 = (400, "User never subscribe this keyword!")
		self.assertEqual(unsubscribe_product("2222", product, type, platform), respond2)  

		# Test currect unsubscribe 
		respond3 = (200, "Unsubscribe successfully!")
		self.assertEqual(unsubscribe_product(uid, product, type, platform), respond3) 
	...
	def test_unsubscribe_product_productID(self):
		...
		self.assertEqual(unsubscribe_product(uid, product, "productName", platform), respond1)  

		# Test unsubscribe with incurrect platform
		respond2 = (400, "Incurrect platform, Amazon or BestBuy, try again.")
		self.assertEqual(unsubscribe_product(uid, product, type, "Amazon1"), respond2)  

		# Test unsubscribe incurrect product_ID
		respond3 = (400, "No record, check product_ID and platform, try again.")
		self.assertEqual(unsubscribe_product(uid, "B00000000", type, platform), respond3)  

		# Test user never subscribe this product 
		uid = "2222"
		respond4 = (400, "User never subscribe this product!")
		self.assertEqual(unsubscribe_product(uid, product, type, platform), respond4)  

		# Test another user subscribe the same product
		uid2 = "1111"
		respond5 = (200, "Unsubscribe successfully!")
		self.assertEqual(unsubscribe_product(uid2, product, type, platform), respond5)  
