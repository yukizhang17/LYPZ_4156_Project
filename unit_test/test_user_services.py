...
class Test_TestUserServices(unittest.TestCase):
	...
	def test_signup_request(self):
		...
		self.assertTrue("_id" in signup_request(user_email, user_password).json())
		self.assertTrue(signup_request(user_email, user_password).json()["statusCode"] == 400)

		application_email = email + "@gmail.com"
		application_password = "LYPZ4156"
		api_key = "6f074b5f90a147e78988ca4ee373191f"
		self.assertTrue("_id" in signup_request(application_email, application_password, api_key).json())

	#Testcase 3:
	def test_login_request(self):
		username = "test1100@columbia.edu_apikey_6f074b5f90a147e78988ca4ee373191f"
		user_password = "LYPZ4156"

		self.assertTrue("access_token" in login_request(username, user_password))

		user_password = "LYPZ4155"
		self.assertTrue(login_request(username, user_password)["code"] == "403")
